## Mission and issue

- Issue, or direct out-of-Issue authorization:
- Mission:
- Risk class:
- Base commit:

## Outcome

Describe the result and the user- or system-visible behavior.

## Scope

- Allowed paths:
- Changed paths:
- Non-goals:

## Verification

- [ ] One non-fail-fast local incremental CI batch completed
- [ ] Local CI checkpoint/summary identifies the exact head commit
- [ ] Foundation validation
- [ ] Build and relevant tests
- [ ] Schema and architecture checks
- [ ] Permission and isolation checks when applicable
- [ ] Migration and rollback evidence when applicable
- [ ] Security and safety review when applicable

## AI assistance

- Primary writer:
- Planning:
- Reviewers:
- Model and CLI revisions:
- Skills used:
- Human decisions:

## Evidence

- Result contract:
- Review contract:
- Local CI command and summary:
- GitHub CI run:
- Full CI Milestone (leave `N/A` unless closing a Milestone):
- Artifact digest:
- Remaining risks:

## Merge and cleanup gate

- [ ] Exact canonical ADR-0006 GitHub operation gate is satisfied
- [ ] Remote head branch will be deleted after merge
- [ ] Unreferenced local worktrees, branches, caches, logs, downloads, and failed outputs will be removed
- [ ] No production deployment performed
